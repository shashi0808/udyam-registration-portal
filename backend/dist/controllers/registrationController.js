...
exports.getAllRegistrations = exports.lookupPinCode = exports.submitRegistration = exports.validatePAN = exports.verifyOTP = exports.sendOTP = void 0;
...
const verifyOTP = async (req, res, next) => {
    try {
        ...
        if (!storedData) {
            return next((0, errorHandler_1.createError)('OTP not found. Please request a new OTP.', 400));
        }
        // Check if OTP expired (10 minutes)
        const isExpired = Date.now() - storedData.timestamp > 10 * 60 * 1000;
        if (isExpired) {
            delete otpStore[aadhaarNumber];
            return next((0, errorHandler_1.createError)('OTP has expired. Please request a new OTP.', 400));
        }
        // Verify OTP
        if (storedData.otp !== otp) {
            return next((0, errorHandler_1.createError)('Invalid OTP. Please try again.', 400));
        }
        // Mark as verified
...
const submitRegistration = async (req, res, next) => {
    try {
        ...
        if (!otpData || !otpData.verified) {
            return next((0, errorHandler_1.createError)('Aadhaar OTP verification required before submission.', 400));
        }
        ...
        if (age < 18) {
            return next((0, errorHandler_1.createError)('Applicant must be at least 18 years old.', 400));
        }
...
const lookupPinCode = async (req, res, next) => {
    try {
        const { pincode } = req.params;
        // Validate pincode format
        if (!/^[0-9]{6}$/.test(pincode)) {
            return next((0, errorHandler_1.createError)('Invalid PIN code format', 400));
        }
        try {
            ...
            if (response.data && response.data[0] && response.data[0].Status === 'Success') {
                ...
                return next((0, errorHandler_1.createError)('Invalid PIN code or no data found', 404));
            }
        }
        catch (apiError) {
            ...
            if (mockData) {
                ...
                return next((0, errorHandler_1.createError)('PIN code lookup service temporarily unavailable', 503));
            }
        }
    }
    catch (error) {
